...
- `FN1 + Shift` 를 누르면 `MS_ACL0`(느린 속도)로 전환되어 정밀 이동이 됩니다. 코드 라인/글자
  사이를 정밀하게 오갈 수 있도록 **아주 느리게**(`MK_C_OFFSET_0 = 1`) 설정했습니다. 더 빠르게
  원하면 `users/wanja727/config.h`에서 값을 올리면 됩니다.
